...
4. **Ask before you spend.** On an activated (non-sandbox) tenant, `setup_infrastructure` billing tracks the provisioned mailbox count — call it with `quoteOnly: true` first ("pass `quoteOnly:true` first to preview the new count + projected monthly price before committing (no silent capacity addition)") and show the user the resulting `projectedMonthlyCents` and mailbox/domain counts. Proceed to the real (non-`quoteOnly`) call only after the user explicitly confirms. Any call that would buy a new domain also needs the user's separate, explicit opt-in via `registerDomains: true` — omitting it (or passing `false`) is refused before any spend, and it is the tenant's real consent to a domain purchase, not a default to leave on. This quote-then-confirm flow is the MCP/HTTP `setup_infrastructure` path. The CLI's `agent-cold-email setup` builds a fixed request body — it cannot pass `quoteOnly` or `registerDomains`, and it silently substitutes a placeholder CAN-SPAM physical address when flags are omitted — so do not use the CLI form for this consent step; call `setup_infrastructure` directly over MCP/HTTP instead. The call can take a while to answer since provisioning runs before the response; poll `infrastructure_status` (or re-call it) to track progress.
...
- **Lookalike domains are scoped to the customer's own brand only.** `setup_infrastructure` runs a server-side, code-enforced validator that rejects a well-known-brand denylist and requires the asserted `brand` to correspond to `primaryDomain`. Never use, and never help a user use, the domain generator to impersonate a brand that isn't theirs.
...
- https://github.com/YS-projectcalc/agent-cold-email — public CLI/skills/plugins/integrations source and the full `AGENTS.md` operational contract; the hosted platform itself is not open source.
