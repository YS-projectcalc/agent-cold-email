...
//    the bridge used to be flaky because it awaited the FULL upstream
//    handshake before starting the stdio server.
// 2. The process must exit cleanly on stdin EOF, not linger as an orphan
//    with a dangling upstream connection.
...
test("mcp bridge: initialize answers fast even when the upstream connect is slow", async (t) => {
  // 8s matches the order of magnitude measured against the live
  // production endpoint (4-4.8s) and the stub under load (up to 6.8s) —
  // comfortably reproduces the flake without being contrived.
  const SLOW_MS = 8000;
