...
const CLIENTS: Record<ClientKey, { label: string; note: string; code: string; steps: string[] }> = {
  ...
  cursor: {
    ...
    steps: ["Set COLDRIG_TOKEN where the Cursor desktop process can read it.", "Add the remote server to personal ~/.cursor/mcp.json.", "Restart Cursor and inspect its 27 tools; if bearer auth fails, do not hardcode the token in a project file."],
  },
  cline: {
    label: "Cline",
    ...
    steps: ["Open MCP Servers → Configure, or edit personal ~/.cline/mcp.json.", "Add the remote server and authorization header.", "Connect, verify all 27 tools, and approve calls individually during the sandbox."],
  },
};
