...
There is ALSO a repo-local `MEMORY.md` at repo root (confirmed 2026-08-20, ~30KB) — listed explicitly as canonical in the repo's own `CLAUDE.md` ("milestone events also update MEMORY.md"), distinct from Yaakov's global cross-session memory system. Convention: one dated bullet per milestone, appended at the end of the file (no headers/sections beyond a few dated `##` grouping headers early in the file's history) — commit hash(es), Worker ID, gate round summary, battery numbers, one CLASS lesson if the wave surfaced one. Match this exact density; don't add new subheadings.

Verified 2026-08-20 (B.2+sweepcal ship pass): `wrangler` CLI works from repo root (reads root `wrangler.toml`) for `deployments list` and `d1 migrations list <db-name> --remote` (the db name positional is REQUIRED — bare `d1 migrations list --remote` prints the help text, not an error, so a silent no-op looks like output). `deployments list` prints oldest-first; use `tail`, not `head`, to see the latest. Both are fast, safe, read-only spot-checks worth running before trusting a brief's Worker-ID/migration claim verbatim.

`ACTIVATION.md` (repo root) is also canonical — the owner-hands checklist, organized `## Gate 0` (decisions only Yaakov can make) through `## Gate 4` (legal/ops arming), each item a `- [ ]`/`- [x]` line with WHY-not-automated + WHAT's-wired. Corrections to a Gate item get appended inline as a **CORRECTED YYYY-MM-DD (...)** sentence right after the original claim, not a rewrite — same convention as ROADMAP `## Open`. Cross-reference ROADMAP's `## Open` entries rather than duplicating their evidence bodies (2026-07-20 vendor-ruling propagation confirmed this pattern works cleanly across ACTIVATION/ROADMAP/SPEC/HANDOFF/MEMORY in one pass).
