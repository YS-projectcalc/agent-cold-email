...
describe("warmup auto-cancel at ramp completion (founder ruling 2026-08-02)", () => {
  ...
  it("A1 — the CRON entry point reaches the sweep (this is the only production driver)", async () => {
    ...
    // window. Reaching every tenant across `ceil(total / slice)` ticks IS the
    // bounded sweep's guarantee, and this test is about whether the cron drives
    // the sweep at all — never about which tick does it.
