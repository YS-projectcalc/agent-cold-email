...
export const ALLOWED_UNISOLATED_LOOPS: { file: string; header: string; reason: string }[] = [
  ...
  // --- STILL OPEN: doc-named IN/UNCERTAIN members no commit on this branch closed. ---
  ...
  //
  // IN-6 (watchtower.ts reconcileAlerts) was on this list and is GONE from it:
  // the channel-truth lane closed it with a real per-result try/catch that
  // records an `unreportable` outcome and carries on, so the loop no longer
  // reaches the scan as an offender. Keeping the entry would have been the
  // stale-claim failure the third assertion exists to catch, and deleting it
  // restores the tripwire over that loop — remove the catch and it is flagged.
  {
