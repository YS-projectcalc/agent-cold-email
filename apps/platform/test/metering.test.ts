...
// B1 brief: "Ensure per-mailbox/mo metering aggregates correctly into
// account().usageCents." Paid-tier only (SEE engine/provisioning.ts comment)
// — demo/free's usageCents stays 0, proven as a regression by the UNCHANGED
// e2e.test.ts assertion (usageCents === 0 for a demo tenant, no mailbox fee
// and no per-send fee).
describe("metering aggregates per-mailbox/mo fees for a paid tenant", () => {
  it("adds a mailbox provisioning fee at setup time, and a tick adds no further usage", async () => {
    ...
    // Sends carry no fee — a tick must not add to usageCents.
    const afterSend = await api<AccountResponse>("/account", { token });
    expect(afterSend.body.usageCents).toBe(1200);
  });
