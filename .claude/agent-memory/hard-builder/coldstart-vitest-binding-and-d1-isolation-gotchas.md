...
5. **A SqlStorage row generic must be a `type` alias, not an `interface`** — `sql.exec<Row>(...)` requires `Record<string, SqlStorageValue>`, and an `interface` gets no implicit index signature, so it fails `TS2344: Index signature for type 'string' is missing`. Every existing row shape in the suite is a `type` for this reason; the tests still RUN green (vitest transpiles without typechecking), so only `npm run typecheck` catches it — run it before claiming done.
6. **`import.meta.glob("./*.test.ts", { query: "?raw", import: "default", eager: true })` works in vitest-pool-workers** — the way to write a tripwire that scans EVERY test source (a per-file explicit `?raw` import list silently misses the next file added).
