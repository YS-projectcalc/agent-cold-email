...
The installable Coldrig Claude Code plugin — its own small root, separate from the repo root, so `claude plugin install coldrig@coldrig` copies only this directory (a few KB) into the plugin cache instead of the whole repo.
