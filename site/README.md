...
- `guide-mcp-tool-count.html` — compares smaller intent-level and larger granular MCP surfaces; maps Coldrig's 27 tools to its documented lifecycle and states the control/webhook tradeoffs.
...
- `openapi.yaml` — the 27 facade intents (core pipeline + the optional dashboard session/views surface, outbound webhook subscriptions, and lead disposition/suppression/export) as an OpenAPI 3.1 REST spec, matching the platform's committed route and schema definitions exactly.
