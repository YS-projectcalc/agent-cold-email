...
- It is **live**: CF Email Sending + Routing for `coldrig.dev` were armed and end-to-end-verified 2026-07-20 (`ROADMAP.md:92`, `ACTIVATION.md` Gate 4). DKIM is Cloudflare-managed for the onboarded domain (domain-wide signing); SPF/return-path are set by CF's envelope. So DKIM/SPF alignment for `@coldrig.dev` mail is in place.
