...
1. `setup_infrastructure` — provisions domains + mailboxes, starts (simulated, sandbox) warmup; the call itself can take a while to answer, since provisioning runs before the response. Returns `202` while work is still owed (`200` if it finished within this call); poll `infrastructure_status`.
...
Full tool list, exact schemas, and HTTP mappings: [`AGENTS.md`](./AGENTS.md). REST contract: [coldrig.dev/openapi.yaml](https://coldrig.dev/openapi.yaml).
