...
4. **Single injected Clock.** No direct wall-clock reads anywhere (CI lint gate). Warmup ramp + send scheduling run on a DO-alarm scheduler driven by the injected clock — so weeks-long warmup is testable in minutes AND stays honest. NOT Cloudflare Workflows (can't be virtual-clocked).
   **The clock law (wave-2 DECISION 2, founder ruling 2026-08-05).** `RealClock` for every non-sandbox (paid) tenant; `VirtualClock` is sandbox/demo-only — every tenant signs up as `plan='demo'` and therefore starts on a `VirtualClock`, whose offset jumps forward on each accelerated demo run. Checkout upgrades `plan` but never used to touch the clock, so an unmigrated paying tenant's frozen "now" is typically far in the real FUTURE. A **one-shot, per-tenant-DO migration** (`engine/clock-migration.ts`'s `migrateTenantClockToReal`, guarded by `tenant_profile.clock_mode != 'real'`) rebases every clock-stamped row by `realNow - frozenNow` (a persisted delta, either sign — `clock_migration_delta_ms`/`clock_migrated_at`), retires the tenant's sandbox-origin `mailboxes` AND `domains` rows (nothing exists at any real vendor for them), and stamps `clock_mode='real'` — all inside ONE `storage.transactionSync`, so a throw rolls back everything including the marker and a retry re-enters virgin state. `clock_mode='real'` is also the interlock the wave-2 auto-send driver's gate requires (SPEC.md §23), so an unmigrated tenant is structurally unreachable by automatic sending. A **`DelegatingClock`** (`clock.ts`) wraps a live `TenantContext`'s clock accessor so an already-in-flight saga that captured its context BEFORE the flip (e.g. a multi-minute `setup_infrastructure`) reads REAL time on its very next `clock.now()` call after the migration commits — no cache invalidation needed, since the cached sandbox adapter bundle closes over the delegate, not a frozen clock instance.
